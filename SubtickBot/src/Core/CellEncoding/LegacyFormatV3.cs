...
namespace Indev2.Encoder
{
    public class LegacyFormatV3 : ILevelFormat
    {
        ...
        private static readonly Dictionary<int, int> V3IdMap = new()
        {
            { 0, 2 },
            { 1, 3 },
            { 2, 4 },
        ...
        private static readonly Dictionary<char, int> DecodeDictionary;

        static LegacyFormatV3()
        {
            DecodeDictionary = new Dictionary<char, int>();

            for (var i = 0; i < 74; i++) DecodeDictionary.Add(CellKey[i], i);
        }
        ...
        public (byte[], string) EncodeLevel(ILevel level)
        {
            ...
            //Used variables
            var width = level.Properties.Width;
            var height = level.Properties.Height;
            ...
            //Encode cells
            foreach (var cell in level.CellGrid.GetCells())
            {
                var cellType = cell.Instance.Type;

                if (cellType > 8)
                    continue; //Skip if not a cell (or if it's a cell that doesn't exist in V3
                ...
                cellData[cell.Transform.Position.x + cell.Transform.Position.y * width] +=
                    2 * type + 18 * cell.Transform.Direction.AsInt - 72;
            }
            ...
            {
                maxMatchLength = 0;
                for (var matchOffset = 1; matchOffset <= dataIndex; matchOffset++)
                {
                    matchLength = 0;
        ...
        public static DecodeResult DecodeV3(string input)
        {
            ...
            var width = DecodeString(args[1], DecodeDictionary);
            var height = DecodeString(args[2], DecodeDictionary);

            var cellDataHistory = new int[width * height];
            int offset;
            ...
            while (dataIndex < args[3].Length)
            {
                if (args[3][dataIndex] == ')' || args[3][dataIndex] == '(')
                {
                    ...
                    {
                        dataIndex += 2;
                        offset = DecodeDictionary[args[3][dataIndex - 1]];
                        length = DecodeDictionary[args[3][dataIndex]];
                    }
                    else
                    {
                        dataIndex++;
                        ...
                        offset = DecodeString(temp, DecodeDictionary);
                        if (args[3][dataIndex] == ')')
                        {
                            dataIndex++;
                            length = DecodeDictionary[args[3][dataIndex]];
                        ...
                        else
                        {
                            ...
                            length = DecodeString(temp, DecodeDictionary);
                        }
                    }
                ...
                {
                    var cell = GetV3Cell(DecodeDictionary[args[3][dataIndex]], gridIndex, sizeVector);
                    if (cell.Item1 != null)
                        cells.Add(cell.Item1.Value);
                    if (cell.Item2 != null)
                        dragSpots.Add(cell.Item2.Value);
                    cellDataHistory[gridIndex] = DecodeDictionary[args[3][dataIndex]];
                    gridIndex++;
                }
